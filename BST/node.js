...
function Tree() {
  ...
  this.addNode = function(node) {
    this.data.push(node);//pushed node data into data variable
...
//   }
 ...
 //binary tree creation
  ...
  this.createBinaryTree = function(arr) {
    ...
    start = treeContainer.attr("width") / 2;//i.e the creation starts from mid and we do spacing in it like from 360 to left and right using x and y spacing 

    let i = 0;
    let node = {}; //store an object key value pairs or we can say we are creating an object here
 ...
 //binary search tree starts
  ...
  this.createBinarySearchTree = function(inputArr) {
    ...
    //insert node in the container//

    const insertNode = (arr, depth, cx) => {
      if (!arr.length) { return; }//base condition
  ...
  //binary seaerch tree gets finished

  this.size = function() {
    return d3.selectAll("circle").nodes().length;
  }
}
//function tree get finished


//array create from app.js
...
// purpose of this functon is to create a area(container) for binary tree creation 

function createContainer(id, arr, width, height) {
  let box = calcDimensions(arr);

  let depth = Math.ceil(Math.log2((arr.length - 1) + 2)) - 1 || 1;// 3 level i.e 0,1,2  (calculation for default)

  let container = d3.select(`div#${id}`)
    .append('svg')
    .attr('width', width || box.width * 600 * (.8 / depth) * .75)//for default its 720
    .attr('height', height || box.height) //for default its 400

    //so here we return a container for the binary tree of width 720 and height 400
